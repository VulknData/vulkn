...
class _ASTRewriteChunkByClause():
    ...
    def rewrite(self):
        ...
        for j in range(0, len(self._ast.tokens)):
            if isinstance(self._ast[j], sql.Where):
                self._ast[j].tokens.append(sql.Token('Literal', ' AND ({chunk_clause})'))
                break
            if self._ast[j].ttype == T.Keyword and (
                ...
                self._ast.insert_before(j - 2, sql.Token('Literal', 'WHERE ({chunk_clause})'))
                break
        else:
            self._ast.tokens.append(sql.Token('Literal', 'WHERE AND ({chunk_clause})'))
